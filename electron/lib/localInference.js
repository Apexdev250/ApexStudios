...
function fetchJson(url) {
    return new Promise((resolve, reject) => {
        https.get(url, { headers: { 'User-Agent': 'apexstudios' } }, (res) => {
...
function downloadFile(url, destPath, onProgress) {
    ...
    const attempt = (requestUrl, redirectsLeft, retriesLeft) => new Promise((resolve, reject) => {
        ...
        const reqHeaders = {
            'User-Agent': 'Mozilla/5.0 (compatible; apexstudios/1.0)',
            'Accept': '*/*',
...
const CUSTOM_BINARIES = {
    'darwin-arm64': 'https://github.com/Apexdev250/ApexStudios/releases/download/v1.0.3-binaries/sd-cli-metal-macos-arm64.zip',
};
