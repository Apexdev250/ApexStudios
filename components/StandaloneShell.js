...
const STORAGE_KEY = "apexstudios_api_key";
const STORAGE_PROVIDER_KEY = "apexstudios_api_provider";
...
export default function StandaloneShell() {
  ...
  const getWorkflowInfo = useCallback(() => {
    ...
    const wfIndex = slug.findIndex(
      (s) => s === "workflows" || s === "workflow",
    );
  ...
  const [providerId, setProviderId] = useState("muapi");
  ...
  const fetchBalance = useCallback(
    async (key, pid = providerId) => {
      ...
      try {
        ...
        console.warn("Balance fetch skipped/failed:", err?.message || err);
        setBalance(null);
      }
    },
    [providerId],
  ...
  useEffect(() => {
    ...
    const storedProvider =
      localStorage.getItem(STORAGE_PROVIDER_KEY) || "muapi";
    // Read the provider-specific key, not the common key.
    // This prevents sending a MuAPI key to Alibaba's DashScope (401 error).
    // IMPORTANT: Only fall back to the common key for MuAPI provider.
    // If the provider is Alibaba and the alibaba-specific key doesn't exist,
    // return null so the user is prompted to re-enter their provider-specific key
    // instead of accidentally sending a MuAPI key to DashScope.
    const storedKey = getApiKey(storedProvider) || (storedProvider === 'muapi' ? localStorage.getItem(STORAGE_KEY) : null);

    if (storedKey) {
      setApiKey(storedKey);
      setProviderId(storedProvider);
      setCurrentProvider(storedProvider);
      if (storedProvider === "muapi") fetchBalance(storedKey, storedProvider);
      else setBalance(null);
      document.cookie = `apexstudios_api_key=${storedKey}; path=/; max-age=31536000; SameSite=Lax`;
      document.cookie = `muapi_key=${storedKey}; path=/; max-age=31536000; SameSite=Lax`;
    }
  }, [fetchBalance]);

  const handleKeySave = useCallback(
    (key, pid) => {
      const nextProviderId = pid || "muapi";
      localStorage.setItem(STORAGE_KEY, key);
      localStorage.setItem(STORAGE_PROVIDER_KEY, nextProviderId);
      setApiKey(key);
      setProviderId(nextProviderId);
      setCurrentProvider(nextProviderId);
      if (nextProviderId === "muapi") fetchBalance(key, nextProviderId);
      else setBalance(null);
      document.cookie = `apexstudios_api_key=${key}; path=/; max-age=31536000; SameSite=Lax`;
  ...
  const handleKeyChange = useCallback(() => {
    ...
    setBalance(null);
    document.cookie =
      "apexstudios_api_key=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
    document.cookie =
      "muapi_key=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
  }, []);
  ...
  useEffect(() => {
    if (!apiKey || providerId !== "muapi") return;
    const interval = setInterval(() => fetchBalance(apiKey, providerId), 30000);
    return () => clearInterval(interval);
  }, [apiKey, providerId, fetchBalance]);
  ...
  return (
    <ModelProvider apiKey={apiKey} providerId={providerId}>
      <div
      ...
      >
        {/* Drag Overlay */}
        {isDragging && (
          <div className="fixed inset-0 z-[100] bg-blue-600/10 backdrop-blur-md border-4 border-dashed border-blue-500/50 flex items-center justify-center pointer-events-none transition-all duration-300">
