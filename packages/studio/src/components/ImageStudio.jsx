            ...
            <div className="flex items-center gap-2">
              {isMulti && hasSelection && (
                <button
                  type="button"
                  onClick={handleDone}
                  className="flex items-center gap-1 px-3 py-1.5 bg-primary text-white rounded-xl text-xs font-black transition-all hover:scale-105"
                >
                  ✓ Done ({count})
                </button>
              )}
          ...
          {isMulti && hasSelection && (
            <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={handleDone}
                className="px-4 py-1.5 bg-primary text-white rounded-xl text-xs font-black transition-all hover:scale-105"
              >
                Use Selected
              </button>
            </div>
...
export default function ImageStudio({
  ...
  const { t2iModels: dynamicT2I = [], i2iModels: dynamicI2I = [], useStaticFallback = true, error: modelError, loading: modelsLoading } = useModels();
  const t2iModels = useMemo(() => dynamicT2I.length > 0 ? dynamicT2I : (useStaticFallback ? staticT2I : []), [dynamicT2I, useStaticFallback]);
  const i2iModels = useMemo(() => dynamicI2I.length > 0 ? dynamicI2I : (useStaticFallback ? staticI2I : []), [dynamicI2I, useStaticFallback]);
  ...
  const [selectedAr, setSelectedAr] = useState(
    () => staticT2I[0]?.inputs?.aspect_ratio?.default || "1:1",
  );
  const [selectedQuality, setSelectedQuality] = useState(() => {
    const resolutions = getResolutionsForModel(staticT2I[0]?.id || '');
  ...
  useEffect(() => {
    const list = imageMode ? i2iModels : t2iModels;
    if (list.length === 0) {
      setSelectedModelId('');
      setSelectedModelName('');
      return;
    }
    if (!list.find(m => m.id === selectedModelId)) {
      setSelectedModelId(list[0].id);
      setSelectedModelName(list[0].name);
    }
  }, [t2iModels, i2iModels, imageMode]);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full pt-4 animate-fade-in-up">
            ...
            <div className="flex flex-col items-center justify-center h-full animate-fade-in-up min-h-[50vh]">
              <div className="flex flex-col items-center gap-4 p-8 max-w-md text-center">
                ...
                <p className="text-white/50 text-sm leading-relaxed">{modelError}</p>
                <p className="text-white/30 text-xs">
                  Check that your API key is correct and has access to the selected provider.
                  Open <strong className="text-white/50">Settings</strong> in the top bar to update your key or switch providers.
                </p>
              </div>
            </div>
          ) : modelsLoading ? (
            <div className="flex flex-col items-center justify-center h-full animate-fade-in-up min-h-[50vh]">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-white/40 text-sm">Loading available models...</p>
              </div>
            </div>
          ) : (
        ...
        <div className="w-full bg-[#0a0a0a]/80 backdrop-blur-3xl rounded-md border border-white/10 p-4 flex flex-col gap-2 shadow-2xl">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-2 border-t border-white/[0.03] relative">
              ...
              <div className="relative">
                ...
                >
                  <div className="w-4 h-4 bg-blue-600 rounded flex items-center justify-center">
                    <span className="text-[9px] font-bold text-white uppercase">G</span>
                  </div>
            ...
            <button
              ...
              className="bg-blue-600 text-white px-4 py-2 rounded-md font-medium text-sm hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 w-full sm:w-auto shadow-lg shadow-blue-600/10 disabled:opacity-50 disabled:cursor-not-allowed z-10"
            >
              {generating ? (
                <>
