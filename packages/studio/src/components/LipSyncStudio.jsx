...
export default function LipSyncStudio({
  ...
  const { lipsyncModels: dynamicLipSync = [], error: modelError, loading: modelsLoading, useStaticFallback = true } = useModels();
  const lipsyncModels = dynamicLipSync.length > 0 ? dynamicLipSync : (useStaticFallback ? staticLipSync : []);
  // Recompute derived lists from dynamic models
  const imageLipSyncModels = useMemo(
    () => lipsyncModels.filter(m => m.category === 'image' || !m.category),
    [lipsyncModels],
  );
  const videoLipSyncModels = useMemo(
    () => lipsyncModels.filter(m => m.category === 'video'),
    [lipsyncModels],
  );
  ...
  useEffect(() => {
    ...
    if (!currentModels.find(m => m.id === selectedModelId)) {
      setSelectedModelId(first.id);
      setSelectedResolution(
        first.inputs?.resolution?.default ?? first.inputs?.resolution?.enum?.[0] ?? "480p",
      );
    }
  }, [currentModels, inputMode]);
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-2 border-t border-white/[0.03] relative">
            <div className="flex items-center gap-2 px-1">
              ...
              <div className="relative">
                ...
                >
                  <div className="w-3.5 h-3.5 bg-blue-600 rounded-sm flex items-center justify-center"><span className="text-[9px] font-black text-white">
                      S
                    </span>
                  </div>
