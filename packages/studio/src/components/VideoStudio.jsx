...
import { useModels } from '../ModelContext.js';
...
const VideoReadySvg = () => (
  ...
  >
    <polygon points="23 7 16 12 23 17 23 7" />
    <rect x="1" y="5" width="15" height="14" rx="2" ry="2" />
    <polyline points="7 10 10 13 15 8" stroke="#60a5fa" strokeWidth="2.5" />
  </svg>
);
...
function ModelDropdown({ t2vModels, i2vModels, v2vModels, imageMode, selectedModel, onSelect, onClose }) {
...
export default function VideoStudio({
  ...
  const { t2vModels: dynamicT2V = [], i2vModels: dynamicI2V = [], v2vModels: dynamicV2V = [], error: modelError, loading: modelsLoading, useStaticFallback = true } = useModels();
  const t2vModels = useMemo(() => dynamicT2V.length > 0 ? dynamicT2V : (useStaticFallback ? staticT2V : []), [dynamicT2V, useStaticFallback]);
  const i2vModels = useMemo(() => dynamicI2V.length > 0 ? dynamicI2V : (useStaticFallback ? staticI2V : []), [dynamicI2V, useStaticFallback]);
  const v2vModels = useMemo(() => dynamicV2V.length > 0 ? dynamicV2V : (useStaticFallback ? staticV2V : []), [dynamicV2V, useStaticFallback]);
  ...
  // ── model / params ──
  const defaultModel = t2vModels[0] || (useStaticFallback ? staticT2V[0] : null);
  ...
  }, [imageMode, v2vMode, t2vModels, i2vModels, v2vModels]);
  ...
  // ── Reset to first model if current selection is no longer valid ──────────
  useEffect(() => {
    const list = v2vMode ? v2vModels : (imageMode ? i2vModels : t2vModels);
    if (list.length === 0) {
      setSelectedModel('');
    ...
    if (!list.find(m => m.id === selectedModel)) {
      ...
      setSelectedModelName(list[0].name);
      applyControlsForModel(list[0].id, imageMode, v2vMode);
    }
  }, [t2vModels, i2vModels, v2vModels, imageMode, v2vMode]);
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto overflow-y-auto custom-scrollbar pb-40 lg:pb-32 px-2">
        {history.length > 0 ? (
                ...
                >
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-2 bg-black/60 backdrop-blur-md rounded-full text-white hover:bg-blue-700 transition-all border border-white/10"
                    >
          ...
          <div className="flex flex-col items-center justify-center h-full animate-fade-in-up transition-all duration-700 min-h-[50vh]">
            <div className="w-full max-w-md bg-red-500/5 border border-red-500/20 rounded-2xl p-8 flex flex-col items-center text-center gap-4">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-white font-bold text-sm">Failed to load models</span>
                <span className="text-white/40 text-xs leading-relaxed">{modelError}</span>
              </div>
              <p className="text-white/30 text-xs">Open <strong>Settings</strong> to update your API key or switch providers.</p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center h-full animate-fade-in-up transition-all duration-700 min-h-[50vh]">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-2 border-t border-white/[0.03] relative">
            <div className="flex items-center gap-2 relative flex-wrap pb-1 md:pb-0">
              ...
              <div className="relative">
                ...
                >
                  <div className="w-4 h-4 bg-blue-600 rounded flex items-center justify-center shadow-lg shadow-blue-600/10">
                    <span className="text-[9px] font-bold text-white uppercase">
                      V
                    </span>
                  </div>
                  <span className="text-xs font-semibold text-white/70 group-hover:text-blue-400 transition-colors">
                    {selectedModelName}
                  </span>
                  <svg
                    width="8"
              ...
              {showAr && (
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="text-[11px] font-semibold text-white/70 group-hover:text-blue-400 transition-colors">
                      {selectedAr}
                    </span>
                  </button>
                  {openDropdown === "ar" && (
            ...
            <button
              ...
              className="bg-blue-600 text-white px-4 py-2 rounded-md font-medium text-sm hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 w-full sm:w-auto shadow-lg shadow-blue-600/10 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {modelsLoading ? (
                <>
