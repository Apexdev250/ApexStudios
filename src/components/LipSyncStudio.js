...
import { getLipSyncModelById, getResolutionsForLipSyncModel } from '../lib/models.js';
...
export function LipSyncStudio() {
    ...
    let inputMode = 'image';
    let currentLipSyncModels = [];
    let currentImageLipSyncModels = [];
    let currentVideoLipSyncModels = [];
    let selectedModel = '';
    ...
    const loadModels = () => {
        ...
        currentLipSyncModels = models.lipsyncModels || [];
        currentImageLipSyncModels = currentLipSyncModels.filter(m => m.type === 'image');
        currentVideoLipSyncModels = currentLipSyncModels.filter(m => m.type === 'video');

        if (currentImageLipSyncModels.length > 0) {
            selectedModel = currentImageLipSyncModels[0].id;
            selectedResolution = currentImageLipSyncModels[0].inputs?.resolution?.default || '480p';
        }
    };
    ...
    const getCurrentModels = () => inputMode === 'image' ? currentImageLipSyncModels : currentVideoLipSyncModels;
    const getCurrentModel = () => currentLipSyncModels.find(m => m.id === selectedModel);
