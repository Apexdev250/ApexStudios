...
import {
    ...
    getAspectRatiosForI2IModel, getResolutionsForI2IModel, getQualityFieldForI2IModel,
...
export function ImageStudio() {
    ...
    const initializeWithFallback = () => {
        currentModels = imageMode ? [] : []; // Will be populated by global model manager
        defaultModel = null;
        selectedModel = '';
        selectedModelName = '';
        selectedAr = '1:1';
    ...
    const loadModels = () => {
        ...
        currentModels = imageMode ? models.i2iModels : models.t2iModels;
        
        if (currentModels.length > 0) {
            defaultModel = currentModels[0];
            selectedModel = defaultModel.id;
            selectedModelName = defaultModel.name;
            selectedAr = defaultModel.inputs?.aspect_ratio?.default || '1:1';
        } else {
    ...
    globalModelManager.addListener((models) => {
        ...
        if (currentModels.length > 0) {
            const currentModelExists = currentModels.some(m => m.id === selectedModel);
            if (!currentModelExists) {
                ...
                const arBtnLabel = document.getElementById('ar-btn-label');
                if (arBtnLabel) arBtnLabel.textContent = selectedAr;
            }
        }
    });
    ...
    const getCurrentModels = () => currentModels;
    ...
    // Guard against models being null (loaded async from API)
    const _initResolutions = defaultModel ? getResolutionsForModel(defaultModel.id) : [];
                ...
                if (useLocalModel) {
                    ...
                    filtered.forEach(m => {
                        ...
                        item.innerHTML = `
                            ...
                            ${selectedLocalModel === m.id ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                        `;
                        item.onclick = (e) => {
                            e.stopPropagation();
            ...
            options.forEach(opt => {
                ...
                item.innerHTML = `
                    <span class="text-xs font-bold text-white opacity-80 group-hover:opacity-100">${opt}</span>
                     ${document.getElementById('quality-btn-label').textContent === opt ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                `;
                item.onclick = (e) => {
                    e.stopPropagation();
