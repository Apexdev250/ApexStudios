...
import { globalModelManager, v2vModels } from '../lib/globalModelManager.js';
...
export function VideoStudio() {
    ...
    // --- State ---
    let currentT2VModels = [];
    let currentI2VModels = [];
    let allT2V = [];
    let allI2V = [];
    ...
    // Load models from global model manager
    const loadModels = () => {
        const models = globalModelManager.getModels();
        currentT2VModels = models.t2vModels || [];
        currentI2VModels = models.i2vModels || [];

        // Merge local Wan2GP models
        const localT2V = isLocalAIAvailable() ? localT2VModels.map(adaptLocalToVideoEntry) : [];
        const localI2V = isLocalAIAvailable() ? localI2VModels.map(adaptLocalToVideoEntry) : [];
        allT2V = [...currentT2VModels, ...localT2V];
        allI2V = [...currentI2VModels, ...localI2V];

        if (allT2V.length > 0) {
            defaultModel = allT2V[0];
    ...
    globalModelManager.addListener((models) => {
        loadModels();
        // Update model selection if current model is no longer available
        if (allT2V.length > 0) {
            const currentModelExists = allT2V.some(m => m.id === selectedModel);
            if (!currentModelExists) {
                ...
                const modelBtnLabel = document.getElementById('v-model-btn-label');
                if (modelBtnLabel) modelBtnLabel.textContent = selectedModelName;
            }
        }
    });
    ...
    const getCurrentModels = () => v2vMode ? v2vModels : (imageMode ? currentI2VModels : currentT2VModels);
    ...
    const picker = createUploadPicker({
        ...
        onSelect: ({ url }) => {
            ...
            if (!imageMode) {
                imageMode = true;
                selectedModel = currentI2VModels[0].id;
                selectedModelName = currentI2VModels[0].name;
        ...
        onClear: () => {
            ...
            imageMode = false;
            selectedModel = currentT2VModels[0].id;
            selectedModelName = currentT2VModels[0].name;
            document.getElementById('v-model-btn-label').textContent = selectedModelName;
            updateControlsForModel(selectedModel);
    ...
    videoReadyEl.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-primary"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/><polyline points="7 10 10 13 15 8" stroke="#60a5fa" stroke-width="2.5"/></svg>`;
        ...
        if (type === 'model') {
            ...
            const makeModelItem = (m, isV2V = false) => {
                ...
                item.innerHTML = `
                    ...
                    ${selectedModel === m.id ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                `;
                item.onclick = (e) => {
                    e.stopPropagation();
                    if (isV2V) {
            ...
            const renderModels = (filter = '') => {
                ...
                const generationModels = imageMode ? currentI2VModels : currentT2VModels;
            ...
            availableArs.forEach(r => {
                ...
                item.innerHTML = `
                    <div class="flex items-center gap-4">
                     ...
                     ${selectedAr === r ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                `;
                item.onclick = (e) => {
                    e.stopPropagation();
                    selectedAr = r;
            ...
            durations.forEach(d => {
                ...
                item.innerHTML = `
                    <span class="text-xs font-bold text-white opacity-80 group-hover:opacity-100">${d}s</span>
                     ${selectedDuration === d ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                `;
                item.onclick = (e) => {
                    e.stopPropagation();
                    selectedDuration = d;
            ...
            getQualitiesForModel(selectedModel).forEach(q => {
                ...
                item.innerHTML = `
                    ...
                    ${selectedQuality === q ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                `;
                item.onclick = (e) => {
                    e.stopPropagation();
                    selectedQuality = q;
            ...
            resolutions.forEach(r => {
                ...
                item.innerHTML = `
                    <span class="text-xs font-bold text-white opacity-80 group-hover:opacity-100">${r}</span>
                     ${selectedResolution === r ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                `;
                item.onclick = (e) => {
                    e.stopPropagation();
            ...
            getCurrentModes(selectedModel).forEach(m => {
                ...
                item.innerHTML = `
                    ...
                    ${selectedMode === m ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                `;
                item.onclick = (e) => {
                    e.stopPropagation();
                    selectedMode = m;
            ...
            getEffectNamesForModel(selectedModel).forEach(e => {
                ...
                item.innerHTML = `
                    ...
                    ${selectedEffectName === e ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="4"><polyline points="20 6 9 17 4 12"/></svg>' : ''}
                `;
                item.onclick = (ev) => {
                    ev.stopPropagation();
