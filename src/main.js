...
async function initializeApp() {
    try {
        // Check if API key exists using provider-aware helpers
        const provider = localStorage.getItem('apexstudios_api_provider') || 'muapi';
        const apiKey = localStorage.getItem(`apexstudios_api_key_${provider}`) || localStorage.getItem('muapi_key');
        if (apiKey) {
            await globalModelManager.loadModelsFromAPI();
        }
    } catch (error) {
        console.warn('Failed to load models from API:', error);
        // Continue with empty models - they'll be loaded when user saves API key
    }
}
